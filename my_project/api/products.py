...
from my_project.database.models import Product
from my_project.database.schema import ProductInputSchema, ProductOutSchema
...
async def update_product(product_id: int, product: ProductInputSchema,
    ...
    db.commit()
    db.refresh(product_db)
    return {'massage': 'Продукт озгорулду'}
...
async def delete_product(product_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {'massage': 'Продукт удалить болду'}